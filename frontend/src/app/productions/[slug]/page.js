          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {chunk[0] && (
              <motion.div className="relative md:col-span-2 aspect-square rounded-lg overflow-hidden group" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.6 }}>
                ...
                <div className="absolute inset-0 bg-[rgba(0,0,0,0)] group-hover:bg-[rgba(0,0,0,0.1)] transition-all duration-300"></div>
              </motion.div>
            )}
            <div className="space-y-4">
              {chunk[1] && (
...
export default function ProductionPage({ params }) {
  ...
  const { title, subtitle, description, banner_desktop, banner_mobile, gallery, video_excerpts } = production;
